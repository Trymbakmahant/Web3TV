...
export default function SimplePlayer() {
  ...
  useEffect(() => {
    if(contract){
      ...
      (async function(){
        const provide = new ethers.providers.JsonRpcProvider("https://eth-mainnet.g.alchemy.com/v2/RTRrA6H-niBMX-0b1uGNHyzAYVWBVPfq");
  ...
  const subscribeHandler = async () => {
    const http = axios.create({
      ...
      const subscribers = await http.post(`/smart-contract/polygon-mumbai/${contractAddress}/read`, {
    ...
    if(subscribers.data.response <= 100){
      console.log("here");
      const tx = await contract.addSubscription(creatorId, {value: ethers.utils.parseEther("0.001")});
      await tx.wait();
      console.log("now");
    }else if(subscribers.data.response <= 10000 && subscribers > 100){
      const tx = await contract.addSubscription(creatorId, {value: ethers.utils.parseEther("0.01")});
      await tx.wait();
    }
    ...
    const tx = await contract.mintToken("https://gateway.pinata.cloud/ipfs/bafybeiau2dlojrb4jf3dz2inumz7yel4zy47pjorj3plp4yuuhu6dfycju", {gasLimit: 600000});
    await tx.wait();
    ...
    const tokenData = {
      ...
      "image": 'https://gateway.pinata.cloud/ipfs/bafybeiau2dlojrb4jf3dz2inumz7yel4zy47pjorj3plp4yuuhu6dfycju/silver%20tickit.png',
      "tokenId":(currentToken-1).toString(),
      "description":"You are now a proud member" 
    };

  const properties = [{nftQuality: "silver"}, {visibility:"rare"}, {availability: "15%"}];
  ...
  }else if(subscribers.data.response <= 10000 && subscribers.data.response >100){
    const tokenData = {
        
      "name": 'Member only NFT',
      "image": 'https://gateway.pinata.cloud/ipfs/bafybeifznetfar566yeq2cagadzsriqq4ivrh3yxfz7k74a5mh4yckff7q',
      "tokenId":(currentToken - 1).toString(),
      "description":"You are now a proud member" 
    };

  const properties = [{nftQuality: "Gold"}, {visibility:"legendary"}, {availability: "7.5%"}];
        ...
        <div className="navbar bg-base-100">
          <div className="flex-1">
            <label className="btn btn-ghost btn-circle avatar">
              <div className="w-10 rounded-full">
                <img src={userAvatar ? userAvatar : "https://placeimg.com/80/80/people"} />
              </div>
            </label>
          ...
          <div className="flex-none">
            <div className="dropdown dropdown-end">
              <button className="btn btn-outline btn-success" style = {{marginRight: "30px", marginTop: "15px"}} onClick = {subscribeHandler}>
                Become a Member
              </button>
            </div>
          </div>
        </div>
        <div style = {{marginLeft: "60px", marginBottom: "20px"}}>By {userName ? userName : creatorId}</div>
        <div style={{ marginLeft: "60px" }}>{contract && info.description}</div>
        <h1 style = {{marginLeft: "60px", fontWeight: "600"}}>{totalSubscribers} Subscribers</h1>
